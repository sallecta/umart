...
<?php foreach ($data['packages'] as $extend):
    ...
    <div class="uk-card uk-card-small uk-card-default">
        ...
        <div class="uk-card-body">
			<?php echo $extend['free'] ? JText::_('COM_UMART_FREE_PRODUCT') : $extend['price']; ?>
            <br/>
				...
				<?php if (!$extend['free'] && !$installed): ?>
                    <a href="<?php echo $extend['link']; ?>"
                       ...
                       target="_blank" uk-icon="icon: credit-card">
						<?php echo JText::_('PLG_UMART_MARKET_BUY_NOW'); ?>
                    </a>
				<?php else: ?>
					<?php if ($installed): ?>
						<?php if ($enabled): ?>
							<?php if ($oldVersion): ?>
                                <a href="<?php echo JRoute::_('index.php?option=com_umart&task=market.install&download=' . base64_encode($download) . '&' . $token . '=1', false); ?>"
                                   class="uk-width-1-1 uk-button uk-button-secondary"
                                   uk-icon="cloud-upload">
									<?php echo JText::sprintf('PLG_UMART_MARKET_UPDATE_TO_VERSION_FORMAT', $version); ?>
                                </a>
							<?php else: ?>
                                <span class="uk-text-success uk-text-uppercase" uk-icon="check">
								    <?php echo JText::_('PLG_UMART_MARKET_INSTALLED'); ?>
                                </span>
							<?php endif; ?>
						<?php else: ?>
                            <span class="uk-text-danger uk-text-uppercase" uk-icon="icon: close">
							    <?php echo JText::_('PLG_UMART_MARKET_DISABLED'); ?>
                            </span>
						<?php endif; ?>
					<?php else: ?>
                        <a href="<?php echo JRoute::_('index.php?option=com_umart&task=market.install&download=' . base64_encode($extend['download']) . '&' . $token . '=1', false); ?>"
                           class="uk-width-1-1 uk-button uk-button-primary"
                           uk-icon="icon: bolt">
							<?php echo JText::_('PLG_UMART_MARKET_INSTALL'); ?>
                        </a>
					<?php endif; ?>
				<?php endif; ?>
            </p>
