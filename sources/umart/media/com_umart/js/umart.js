if (!window._umart) {
    window._umart = {
        ...
        ajax: function (url, data, callback, noLoader) {
            var
                loader = _umart.$('#umart_component').length ? _umart.$('#umart_component') : _umart.$('.umart_scope:eq(0)'),
                token = _umart.getData('token'),
                notification = function (message) {
                    message.split('::').forEach(function (msg) {
                        _umart.umartui.notification('<span uk-icon="icon: warning"></span> ' + msg, {
                            status: 'danger',
                            timeout: 3500,
                            pos: 'top-center',
                        });
            ...
            if (data.umartArea instanceof _umart.$) {
                loader = data.umartArea;
                delete data.umartArea;
            }
            ...
            _umart.$.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        ...
                        _umart.$(document).trigger('esAjaxSuccess', [url, data, response]);
                    } else {
        ...
        lang: {
            ...
            load: function (data) {
                for (var d in data) {
                    _umart.lang.data[d] = data[d];
                }
            },
            _: function (string) {
                var key = _umart.$.trim(string.toUpperCase());
                return _umart.lang.data[key] !== undefined ? _umart.lang.data[key] : string;
            }
        },
        events: {
            getProductPriceTemplate: function () {
                ...
                tr.html(
                    ...
                    '<td>' + _umart.getData('currency_box') + '</td>' +
                    '<td><div class="flatpickr uk-position-relative"><a class="uk-form-icon uk-form-icon-flip" uk-icon="icon: close" data-clear></a><input type="text" name="jform[prices][valid_from_date][]" class="datetime-picker uk-input" autocomplete="off" placeholder="' + _umart.lang._('COM_UMART_FROM_DATE') + '" value="" data-input/></div></td>' +
                    '<td><div class="flatpickr  uk-position-relative"><a class="uk-form-icon uk-form-icon-flip" uk-icon="icon: close" data-clear></a><input type="text" name="jform[prices][valid_to_date][]" class="datetime-picker uk-input" autocomplete="off" placeholder="' + _umart.lang._('COM_UMART_TO_DATE') + '" value="" data-input/></div></td>' +
                    '<td><button type="button" class="uk-button uk-button-danger uk-button-small"' +
                    ' onclick="_umart.events.removeParentBox(this, \'tr\');"><i class="fa fa-times"></i></button></td>'
                );

                return tr;
            },
            addProductPrice: function (boxElement) {
                var priceBox = _umart.$(boxElement);
                var rowBox = _umart.events.getProductPriceTemplate();
                priceBox
                    .find('table:eq(0)>tbody')
                    .append(rowBox);
                _umart.initChosen(boxElement);
                priceBox.find('.flatpickr').each(function () {
                    flatpickr(this, {
                        enableTime: true,
                        dateFormat: 'Y-m-d H:i:s',
                        altFormat: _umart.getData('dateTimeFormat'),
                        enableSeconds: true,
                        altInput: true,
                        time_24hr: true,
                        wrap: true,
            ...
            removeParentBox: function (el, boxEl) {
                _umart.$(el).parents(boxEl + ':eq(0)').remove();
            },
            getOptionTemplate: function () {
                ...
                tr.html(
                    ...
                    ' class="uk-input" placeholder="' + _umart.lang._('COM_UMART_OPTION_VALUE') + '"/></td>' +
                    '<td><input type="text" name="jform[params][options][text][]"' +
                    ' class="uk-input" placeholder="' + _umart.lang._('COM_UMART_OPTION_TEXT') + '"/></td>' +
                    '<td><button type="button" class="uk-button uk-button-danger uk-button-small"' +
                    ' onclick="_umart.events.removeParentBox(this, \'tr\');"><i class="fa fa-times"></i></button></td>'
                );
            ...
            addOption: function (boxElement) {
                var optionBox = _umart.$(boxElement);
                var rowBox = _umart.events.getOptionTemplate();
                optionBox
                    .find('table:eq(0)>tbody')
                    .append(rowBox);
            },
            productInit: function (product) {
                var $ = _umart.$, wrap, productId;
                if (product instanceof $) {
                    wrap = product.length > 1 ? product.eq(0) : product;
                    productId = product.data('productId');
                } else {
                    productId = product;
                    wrap = $('.umart_scope [data-product-id="' + productId + '"]:eq(0)');
                }
                ...
                var
                    url = _umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=cart.calculate',
                    data = {
            ...
            printOrder: function (dataOrder, pageTitle) {
                _umart.ajax(_umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=ajax.loadPrintPage', {
                    dataOrder: dataOrder || {},
                    pageTitle: pageTitle
                }, function (response) {
                    var iframeName = ('PrintOrderWindow' + (new Date()).getTime());
                    var iframe = _umart.$('<iframe name=' + iframeName + '>').css({
                        width: '1px',
                        height: '1px',
                        position: 'absolute',
                        left: '-9999px'
                    }).appendTo(_umart.$('body'));
        ...
        cart: {
            reloadModules: function () {
                var $ = _umart.$
                $('.mod-umart-cart[data-module-id]').each(function () {
                    var
                        module = $(this),
                        moduleId = module.data('moduleId');
                    _umart.ajax(
                        _umart.getData('uri').pathRoot + '/index.php?option=com_ajax',
                        {
                            moduleId: moduleId,
                            module: 'umart_cart',
            ...
            addItem: function (pk, quantity, options) {
                var
                    $ = _umart.$,
                    url = _umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=cart.addItem',
                    request = {
                        requestType: 'addToCart',
                        productId: pk,
                        quantity: quantity,
                        options: typeof options === 'object' ? options.serializeArray() : [],
                        umartArea: $('.umart_scope [data-product-id="' + pk + '"]')
                    };
                _umart.ajax(url, request, function (response) {
                    if (response.data.redirect) {
                        ...
                        if ($.trim(response.data.html) !== '') {
                            var container = $(_umart.umartui.container);
                            container.find('[data-cart-modal]').remove();
                            container.append(response.data.html);
                            _umart.umartui.modal('[data-cart-modal]').show();
                        }

                        _umart.cart.reloadModules();
                    }
                });
            },
            update: function (type, pk, quantity, key) {
                var
                    ...
                    url = _umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=cart.update',
                    container = $('[data-cart-modal].uk-open .es-cart-modal-detail').length ? $('[data-cart-modal].uk-open .es-cart-modal-detail') : $(_umart.umartui.container),
                    request = {
                ...
                _umart.ajax(url, request, function (response) {
                    ...
                    _umart.umartui.notification('<span uk-icon="icon: check"></span> ' + response.message, {
                        status: 'success',
                        timeout: 2500,
                        pos: 'top-center'
                    });
            ...
            coupon: function (coupon) {
                var
                    $ = _umart.$,
                    url = _umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=cart.coupon',
                    code = $.trim(coupon),
                ...
                if (!area.length) {
                    area = $('.umart_scope [data-cart-body]');
                    isCheckout = false;
                }
                ...
                _umart.ajax(url, data, function (response) {
                    ...
                    _umart.umartui.notification(icon + ' ' + response.data.message, {
                        status: status,
                        timeout: 2500,
                        pos: 'top-center'
                    });
            ...
            removeCoupon: function (couponId) {
                var area = _umart.$('#es-summary-wrap');
                _umart.ajax(_umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=cart.removeCoupon', {
                    umartArea: area,
                    couponId: couponId
                }, function (response) {
                    area.find('.es-cart-summary').html(_umart.$(response.data).find('.es-cart-summary').html());
                });
            }
        },
        checkout: {
            saveAddress: function () {
                var
                    ...
                    url = _umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=checkout.saveAddress&' + _umart.getData('token') + '=1';
                if (checkout.length) {
                    ...
                    if (fields.es_validate(checkout)) {
                        _umart.ajax(url, fields.serialize(), function (response) {
            ...
            editAddress: function () {
                var
                    $ = _umart.$,
                    checkout = $('#es-checkout-form'),
                    url = _umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=checkout.editAddress&' + _umart.getData('token') + '=1';
                if (checkout.length) {
                    _umart.ajax(url, {}, function (response) {
                        var data = $('<div/>').html(response.data.html);
                        checkout.html(data.find('#es-checkout-form').html())
                        ...
                        if (_umart.getData('jVersion') === 4 && Joomla.Event) {
            ...
            confirm: function () {
                var
                    ...
                    url = _umart.getData('uri') + '/index.php?option=com_umart&task=checkout.confirm&' + _umart.getData('token') + '=1';
                if (checkout.length && data.es_validate()) {
                    _umart.ajax(url, data.serializeArray(), function (response) {
        ...
        toggleView: function (key, button) {
            ...
            if (key === 'list' || key === 'grid') {
                _umart.storage.setData('list.view', key);
            ...
            if (button) {
                ...
                if (grid.length) {
                    setTimeout(function () {
                        _umart.umartui.grid(grid[0]).$emit();
                    }, 300);
                }
            }
        },
        initChosen: function (container) {
            // This method will be overridden by Umart\Html class
        }
    };
    _umart.$(document).on('initUIContainer', function () {
        var
            $ = _umart.$,
            body = $('body'),
            esScope = $('.umart_scope');

        if (_umart.getData('jVersion')) {
            body.addClass('es-detect-jversion-' + _umart.getData('jVersion'));
        }

        _umart.umartui = UIkit;
        ...
        var handleImage = function () {
            var
                ...
                , mediaSets = _umart.getData('mediaSets')
                , mediaSet
                , img
                , dataSrc
                , i
            ...
            $('img[data-image-size]:not(.imageLoaded), img[data-image-size]:not([src])').each(function () {
                ...
                dataSrc && _umart.umartui.img(this, {dataSrc: dataSrc});
            });
        };
        ...
        // Toggle grid view
        $('[data-view-mode] .es-btn-' + _umart.storage.getData('list.view') || 'grid').trigger('click');
        ...
        esScope.on('click', '[data-add-to-cart]', function (e) {
            ...
            _umart.cart.addItem(area.data('productId'), quantity.length ? quantity.val() : 1, options);
        });
        ...
        esScope.on('change', '[data-product-id] [name^="product_option"], [data-product-id] [data-product-quantity]', function () {
            ...
            _umart.events.productInit(productWrap);
        });
        ...
        esScope.on('change', '#es-checkout-form [name="jform[payment_id]"]', function () {
            ...
            _umart.ajax(_umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=checkout.addPayment', {
                paymentId: el.val(),
        ...
        esScope.on('click', '#es-submit-button', async function (e) {
            ...
            if (payment.length) {
                ...
                try {
                    ...
                    _umart.umartui.notification('<span uk-icon="icon: warning"></span> ' + err, {
                        status: 'warning',
                        pos: 'top-center'
                    });
        ...
        esScope.on('change', '#es-checkout-form [name="jform[shipping_id]"]', function () {
            var el = $(this);
            el.parents('li:eq(0)').addClass('active').siblings('li').removeClass('active');
            _umart.ajax(_umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=checkout.addShipping', {
                shippingId: el.val(),
                umartArea: $('#umart_component')
            }, function (response) {
                var data = $('<div/>').html(response.data);
        ...
        $(document).on('click', '.umart_scope #es-button-filter', function () {
            var
                btnFilter = $('.umart_scope .js-stools-container-filters'),
        ...
        $(document).on('click', '.umart_scope a[data-cart-vendor-id]', function (e) {
            e.preventDefault();
            var href = $(this).attr('href').toString();
            var vendorId = parseInt($(this).data('cartVendorId'));

            if (!isNaN(vendorId) && _umart.getData('vendor.isMultiple') === true) {
                if (href.indexOf('?') === -1) {
        ...
        esScope.on('change', '#es-checkout-form [name^="jform[checkoutFields]"]', function (e) {
            ...
            var
                ...
                url = _umart.getData('uri').pathRoot + '/index.php?option=com_umart&task=checkout.calculateCheckoutFieldsPrice&' + _umart.getData('token') + '=1';
            _umart.ajax(url, fields.serialize(), function (response) {
                if (response.success) {
                    ...
                    for (fieldId in response.data.fieldsPrice) {
                        ...
                        if (fieldLbl.length) {
                            $('<div class="checkout-field-price uk-display-inline-block"/>')
                                .html(_umart.currencyFormat(response.data.fieldsPrice[fieldId].price))
                                .appendTo(fieldLbl);
