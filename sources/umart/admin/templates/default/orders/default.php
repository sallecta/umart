...
easyshop('doc')->addScriptDeclaration('   
	Joomla.submitbutton = function(task){		
		if(task == "order.add"){				
			_es.uikit.modal("#es-modal-new").show();
		}else{			
			if(task == "order.createNew"){				
				if(_es.$("#es-modal-new").find("input,select,textarea").es_validate()){
	...
	_es.$(document).ready(function($){		
		$("#jform_user_id").on("change", function(){
		    ...
		    var ajaxUrl = "' . Uri::base(true) . '/index.php?option=com_easyshop&task=order.loadUserFieldData";
		    _es.ajax(ajaxUrl, {userId: userId, "' . Session::getFormToken() . '": 1}, function (response) {
		        $("#es-modal-new .es-billing-form").html(response.data.billing);
		        $("#es-modal-new .es-shipping-form").html(response.data.shipping);
		        _es.initChosen("#es-modal-new");
		    });		    
		});
	});
');
...
<tr>
    ...
    <th class="uk-table-shrink uk-text-nowrap uk-text-center">
		<?php echo HTMLHelper::_('searchtools.sort', 'COM_EASYSHOP_CODE', 'a.order_code', $vars['listDirn'], $vars['listOrder']); ?>
    </th>
    <th class="uk-table-shrink uk-text-nowrap uk-text-center">
    ...
    <tr>
        ...
        <td class="uk-text-center uk-text-nowrap">
            ...
            <a href="#" class="es-order-icon es-print-order"
               ...
               data-page-title="<?php echo htmlspecialchars(Text::sprintf('COM_EASYSHOP_ORDER_PRINT_TITLE_FORMAT', $item->order_code), ENT_COMPAT, 'UTF-8'); ?>"
               uk-tooltip>
