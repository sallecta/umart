...
abstract class JHtmlUmart
{
	...
	public static function getCurrencies($selectText)
	{
		...
		if (null === $currencies)
		{
			...
			if ($selectText)
			{
				$option[] = [
					'value' => '',
					'text'  => Text::_('COM_UMART_CURRENCY_SELECT'),
				];
			}

			$query = plg_sytem_umart_main('db')->getQuery(true)
				->select('a.id AS value, a.name AS text')
				->from(plg_sytem_umart_main('db')->qn('#__umart_currencies', 'a'))
				->where('a.state = 1')
				->order('a.ordering ASC');
			plg_sytem_umart_main('db')->setQuery($query);

			$currencies = array_merge($option, plg_sytem_umart_main('db')->loadObjectList());
		}
		...
		if (!in_array($buttonSelector, $selectorsLoaded))
		{
			...
			_umart.$(document).ready(function($) {
				$(document).on("click", "' . $buttonSelector . '", function(e){
					...
					_umart.events.printOrder(dataOrder || {}, pageTitle);
				});
			});
		');
		}
	...
	public static function framework()
	{
		...
		if (!$framework)
		{
			...
			{
				HTMLHelper::_('umartui.framework');
				HTMLHelper::_('umartui.iconsFramework');
			}

			/** @var Html $html */
			$html = plg_sytem_umart_main(Html::class);
			$html->addCss('umart.default-common.css', UMART_VERSION_HASH);

			if (plg_sytem_umart_main('site'))
			{
				$html->addCss('umart.default-frontend.css', UMART_VERSION_HASH);
			}
			else
			{
				$html->addCss('umart.default-backend.css', UMART_VERSION_HASH);
			}

			$html->addJs('validate.js', UMART_VERSION_HASH)
				->addJs('umart.js', UMART_VERSION_HASH);
		}
	}
	...
	public static function datetimePicker($id, $name, $value = '', array $options = [])
	{
		...
		$htmlClass    = plg_sytem_umart_main(Html::class);
		...
		$noSelectedText = Text::_('COM_UMART_NO_DATETIME_SELECTED');
		$js             = <<<JAVASCRIPT
_umart.$(document).ready(function ($) { 	
    var valueInput = $('#{$id}'); 
...
JAVASCRIPT;
		...
		return plg_sytem_umart_main('renderer')->render($layoutId, $displayData);
	}
...
HTML;
	...
	{
		$title    = '<strong>' . Text::_('JLIB_HTML_CHECKIN', true) . '</strong>: ' . $editorName . '<br />' . plg_sytem_umart_main(Utility::class)->displayDate($checkedTimeOut);
			...
			if ($isEsIcon || $isSvgExt)
			{
				...
				$iconsLoaded[$key] = <<<XHTML
					<svg class="{$icon}" width="{$width}" height="{$height}">
    					<use xlink:href="{$rootUrl}/media/com_umart/images/icons.svg#{$icon}"></use>
					</svg>
XHTML;
