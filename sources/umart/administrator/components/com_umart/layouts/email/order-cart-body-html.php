...
<div class="items">
    <table
		...
		<?php if (!empty($products)): ?>
			<?php foreach ($products as $product):
				...
				if ($images && isset($images[0]) && is_file(UMART_MEDIA . '/' . $images[0]->file_path))
				{
					$image = 'data:' . $images[0]->mime_type . ';base64, ' . base64_encode(file_get_contents(UMART_MEDIA . '/' . $images[0]->file_path));
				}
				else
				{
					$image = null;
	...
	<?php if (!empty($note)): ?>
        <p>
            <strong><?php echo Text::_('COM_UMART_ORDER_NOTE'); ?>:</strong>
			<?php echo $note; ?>
        </p>
	<?php endif; ?>
</div>
