...
class UmartControllerSystem extends BaseController
{
	public function postUpdate()
	{
		try
		{
			...
			{
				opcache_invalidate(JPATH_PLUGINS . '/umart/system/system.php');
			}
	...
	public function fixSchemas()
	{
		...
		if (!$user->core('admin', 'com_umart', false))
		{
			$user->stop();
		}
		...
		try
		{
			$changeSet = ChangeSet::getInstance($db, UMART_COMPONENT_ADMINISTRATOR . '/sql/updates/');
			$changeSet->fix();
		}
		catch (RuntimeException $e)
		{
		...
		if (plg_sytem_umart_main(System::class)->fixSchemas()
			&& is_dir(UMART_COMPONENT_ADMINISTRATOR . '/sql/updates/mysql')
		)
		{
			$files = Folder::files(UMART_COMPONENT_ADMINISTRATOR . '/sql/updates/mysql', '[0-9\.]+\.sql$');
			...
			if ($files)
			{
				...
				$query = $db->getQuery(true)
					...
					->where($db->quoteName('extension_id') . ' = ' . $db->quote(ComponentHelper::getComponent('com_umart')->id));
				$db->setQuery($query)
					->execute();
			}
		}

		$this->setRedirect(Route::_('index.php?option=com_umart&view=system', false));
	}
	...
	public function regenerateThumbnails()
	{
		...
		ob_end_flush();
		$mediaPath = UMART_MEDIA . '/assets/images';
