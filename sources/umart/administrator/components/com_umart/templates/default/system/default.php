...
if (!is_dir(JPATH_SITE . '/media/com_umart/images'))
{
	@JFolder::create(JPATH_SITE . '/media/com_umart/assets/images', 0755);
}
$systemAfterInformation = plg_sytem_umart_main('app')->triggerEvent('onUmartSystemAfterInformation');
$systemInfo             = [
	'PHP version'            => PHP_VERSION,
	'Joomla! version'        => JVERSION,
	'Umart version'       => UMART_VERSION,
	...
	'Media path is writable' => is_writable(JPATH_SITE . '/media/com_umart/assets/images') ? true : false,
];

$ajaxUrl = Route::_('index.php?option=com_umart&task=system.regenerateThumbnails', false);
...
_umart.$(document).ready(function ($) {
    $('#es-regenerate-images').on('click', function (e) {
        e.preventDefault();
        _umart.umartui.modal.confirm('Click OK to confirm and continue progress!').then(function () {
            ...
            try {
                ...
                xhr.onreadystatechange = function () {
                    if (xhr.readyState === 4) {
                        ...
                        _umart.umartui.notification('<span uk-icon="icon: check"></span> Completed!', {status: 'success'});
                        ...
                        if (response.length) {
                            ...
                            if (response.type === 'Error') {
                                _umart.umartui.notification('<span uk-icon="icon: close"></span> ' + response.message, {status: 'warning'});
                            } else {
                                bar.value = response.progress;
                                text.innerText = response.progress + '%';
                                _umart.umartui.notification.closeAll();
                                _umart.umartui.notification('<span uk-icon="icon: thumbnails"></span> ' + response.message, {status: 'success'});
                            }
                        }
                    }
                };
                ...
                _umart.umartui.notification('<span uk-icon="icon: close"></span> ' + Err, {status: 'danger'});
            }
...
<div id="umart_body" class="umartui_width-3-4@m umartui_width-4-5@xl umartui_width-2-3@s">
    <div class="uk-child-width-1-2@s" uk-grid>
        <div>
			...
			<?php if ($layouts = plg_sytem_umart_main(System::class)->getExpiredLayoutsFiles()): ?>
                            ...
                            <div class="uk-accordion-content">
								<?php foreach ($layout as $version => $file): ?>
                                    <div title="<?php echo htmlspecialchars($file); ?>">
										<?php echo str_replace(JPATH_ROOT . '/templates/' . $template . '/html/com_umart', '...', $file) . '?v=' . $version; ?>
                                    </div>
								<?php endforeach; ?>
                            </div>
                        </li>
        ...
        <div>
            ...
            <a href="<?php echo JRoute::_('index.php?option=com_umart&task=system.fixSchemas&' . JSession::getFormToken() . '=1', false); ?>"
            ...
            <p>This tool will re-generate all Umart media's thumbnails that were uploaded via Umart Media
