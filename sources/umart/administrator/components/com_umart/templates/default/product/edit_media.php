...
plg_sytem_umart_main(Html::class)->jui(['sortable']);
$this->fileEnabled = PluginHelper::isEnabled('umart', 'file');
Text::script('COM_UMART_WARNING_NO_IMAGE_SELECTED');
Text::script('COM_UMART_WARNING_NO_FILES_SELECTED');
    ...
    <div id="es-product-media" class="uk-grid-small uk-child-width-1-2@m es-edit-panel" uk-grid>
        <div>
            <div class="uk-panel uk-card uk-card-small uk-card-default uk-card-body es-border">
                <h3 class="uk-heading-bullet uk-text-uppercase">
					<?php echo Text::_('COM_UMART_MEDIA'); ?>
                ...
                <div class="uk-accordion-content">
                    <div class="uk-card uk-card-default uk-card-body uk-background-muted es-border es-input-100">
                        ...
                        <input type="text"
                               ...
                               class="uk-input"
                               placeholder="<?php echo Text::_('COM_UMART_TITLE'); ?>"
                               value="##title##"/>
                        <textarea class="uk-textarea uk-margin-small"
                                  cols="20" rows="3"
                                  placeholder="<?php echo Text::_('COM_UMART_DESCRIPTION'); ?>"
                ...
                <ul id="es-list-image" uk-accordion="multiple: true">
					<?php if (!empty($this->item->images)):
                        ...
                        <li>
							...
							$mime    = $mediaClass->getMimeByFile(UMART_MEDIA . '/' . $media->file_path);
            ...
            <div class="uk-accordion-content">
                <div class="uk-card uk-card-default uk-card-body uk-background-muted es-border es-input-100">
                    ...
                    <textarea class="uk-textarea uk-margin-small"
                              ...
                              placeholder="<?php echo Text::_('COM_UMART_DESCRIPTION'); ?>">##description##</textarea>
                    <div class="uk-margin-small-bottom">
                        <div class="uk-text-bold uk-margin-small-bottom">
                            <span uk-icon="icon: lock"></span>
							<?php echo Text::_('COM_UMART_ACCESS'); ?>
                        </div>
						<?php
		...
		<?php $fileTemplate = ob_get_clean(); ?>
            ...
            <div>
                <div class="uk-panel uk-card uk-card-small uk-card-default uk-card-body es-border">
                    <h3 class="uk-heading-bullet uk-text-uppercase">
						<?php echo Text::_('COM_UMART_FILES'); ?>
...
Text::script('COM_UMART_REMOVE_CONFIRM');
...
jQuery(document).ready(function ($) {
    ...
    $('#es-list-image').on('click', 'a.es-media-remove', function (e) {
        e.preventDefault();
        if (confirm(Joomla.Text._('COM_UMART_REMOVE_CONFIRM'))) {
        ...
        $(this).on('load', function () {
            ...
            contents.find('.file-selected-insert').on('click', function () {
                ...
                if (filesSelected.length < 1) {
                    alert(Joomla.Text._('COM_UMART_WARNING_NO_' + (isFile ? 'FILES' : 'IMAGE') + '_SELECTED'));
                    return false;
                }
                ...
                if (isFile) {
                    for (i = 0, n = filesSelected.length; i < n; i++) {
                        ...
                        _umart.initChosen('#es-list-file');
                    }
                    _umart.umartui.modal('#es-media-modal-file').hide();
                } else {
                    for (i = 0, n = filesSelected.length; i < n; i++) {
                        file = filesSelected.eq(i);
                        fPath = file.parent().data('file');
                    ...
                    _umart.umartui.modal('#es-media-modal-image').hide();
                }
