...
class JFormFieldAddress extends JFormFieldText
{
	...
	protected function getInput()
	{
		...
		$classes  = $this->class;
		$html     = [];
		$html[]   = '<div class="umartui_width-3-5@m es-multi-input es-address1-input umartui_width-1-2">';
		...
		$html[]   = ' placeholder="' . JText::_('COM_UMART_ADDRESS') . '" value="' . htmlspecialchars($address1, ENT_COMPAT, 'UTF-8') . '"/></div>';
		$html[]   = '<div class="umartui_width-2-5@m es-multi-input es-address2-input umartui_width-1-2">';
		$html[]   = '<input type="text" name="' . $this->name . '[1]" id="' . $this->id . '-line2"';
		$html[]   = ' class="field-address uk-input field-address-line2"';
		$html[]   = ' placeholder="' . JText::_('COM_UMART_ADDRESS2') . '" value="' . htmlspecialchars($address2, ENT_COMPAT, 'UTF-8') . '"/></div>';
