...
class UmartModelProducts extends ListModel
{
	...
	public function getItems()
	{
		...
		{
			/** @var $product Product */
			$product = plg_sytem_umart_main(Product::class);
	...
	protected function getListQuery()
	{
		...
		$query->select(
			...
			->join('LEFT', $db->quoteName('#__umart_price_days', 'a2') . ' ON a2.product_id = a.id AND a2.week_day = ' . (int) CMSFactory::getDate()->format('w'))
...
SQL_SELECT
		...
		if (plg_sytem_umart_main('site') && $view !== 'products')
		{
			$query->where('a.display_in_search = 1 AND a.approved = 1');

			if (plg_sytem_umart_main('config', 'hide_product_off_sale', 0))
			{
		...
		if (!empty($tags))
		{
			...
			if (Translator::isTranslatable())
			{
				$transQuery = $db->getQuery(true)
					...
					->where('a.translationId LIKE ' . $db->quote($langTag . '.umart_tags.%.alias'))
			...
			$query->join('INNER', $db->quoteName('#__umart_tag_items', 'tagItem') . ' ON tagItem.item_id = a.id')
				->join('INNER', $db->quoteName('#__umart_tags', 'tag') . ' ON tag.id = tagItem.tag_id')
				->where('tag.context = ' . $db->quote('com_umart.product'))
