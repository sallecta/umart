...
class UmartModelMethods extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__umart_methods', 'a'));
		...
		$group         = 'umart' . $this->getState('filter.type');
		$umartGroup = ['umartshipping', 'umart_payment'];
		$query
			->select('e.folder')
			->leftJoin($db->quoteName('#__extensions', 'e') . ' ON e.extension_id = a.plugin_id')
			->where('e.type = ' . $db->quote('plugin'));

		if (empty($group) || !in_array($group, $umartGroup))
		{
			$umartGroup = array_map(function ($g) {
				return plg_sytem_umart_main('db')->q($g);
			}, $umartGroup);
			$query->where('e.folder IN (' . join(',', $umartGroup) . ')');
		}
		else
		{
	...
	public function getMethods()
	{
		...
		if ($group && in_array($group, ['umartshipping', 'umart_payment']))
		{
			$group = $db->quote($group);
		}
		else
		{
			$group = $db->quote('umartshipping') . ',' . $db->quote('umart_payment');
		}
