...
class UmartModelOrders extends ListModel
{
	...
	public function getFilterForm($data = [], $loadData = true)
	{
		...
		{
			if (!plg_sytem_umart_main('config', 'multi_currencies_mode', 0))
			{
	...
	protected function getListQuery()
	{
		...
		$query->select($this->getState('list.select', 'a.id, u.name AS author, uu.name AS editor'))
			...
			->leftJoin($db->quoteName('#__umart_users', 'eu') . ' ON eu.id = a.user_id')
		...
		{
			$query->innerJoin($db->quoteName('#__umart_order_products', 'a2') . ' ON a2.order_id = a.id')
