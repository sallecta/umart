...
class Factory
{
	...
	public function dispatch()
	{
		...
		$isAdmin = plg_sytem_umart_main('administrator');
		...
		plg_sytem_umart_main(Classes\Html::class)->initChosen();
		...
		{
			$config['base_path'] = UMART_COMPONENT_ADMINISTRATOR;
		}

		$app->triggerEvent('onUmartBeforeDispatch', [&$config]);
		ob_start();
		$controller = BaseController::getInstance('Umart', $config);
		$controller->execute($app->input->get('task', null, 'CMD'));
		$buffer = ob_get_clean();
		$app->triggerEvent('onUmartAfterDispatch', [&$buffer]);
		echo '<div id="umart_component" class="umartui_scope umart_scope">' . $buffer . '</div>';
	...
	public function getClass($className, $config = [], $namespace = 'Umart', $constructor = null)
	{
	...
	public function __get($name)
	{
		...
		throw new RuntimeException('Cannot access protected property class Umart\\Classes\\Factory::' . $name);
	}
	...
	public function addLangText($keys = [])
	{
		...
		$this->doc->addScriptDeclaration('_umart.lang.load(' . json_encode($data) . ');');
	}

	public function getModel($name, $basePath = UMART_COMPONENT, $config = ['ignore_request' => true])
	{
		static $includePaths = [];

		if (!in_array($basePath, $includePaths))
		{
			BaseDatabaseModel::addIncludePath($basePath . '/models', 'UmartModel');
			$includePaths[] = $basePath;
		}

		return BaseDatabaseModel::getInstance(ucfirst($name), 'UmartModel', $config);
	}
	...
	public function getConfig($name = null, $default = null)
	{
		...
		{
			$config = clone ComponentHelper::getParams('com_umart');
	...
	public function getPlugin($name, $group = 'umart')
	{
		static $plugins = [];
