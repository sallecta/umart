...
class Form extends CMSForm
{
	...
	public function __construct($name, array $options = [])
	{
		...
		$this->translationForm = new CMSForm('com_umart.translation', ['control' => 'UmartTranslations']);
	}
	...
	public function getLanguagesList($ignoreDefault = false)
	{
		...
		if (null === $languages)
		{
			plg_sytem_umart_main(Html::class)->addCss('multi-language.css');
	...
	public function getInput($name, $group = null, $value = null)
	{
		...
		{
			if ($this->isMultiLanguage && 'true' === $field->getAttribute('UmartMultiLanguage'))
			{
	...
	protected function renderMultiLanguageInput(FormField $field)
	{
		...
		HTMLHelper::_('umartui.openTab', 'es-language-tab-' . $name);

		foreach ($this->languagesList as $langCode => $language)
		{
			HTMLHelper::_('umartui.addTab', HTMLHelper::_('image', 'mod_languages/' . $language->image . '.gif', '', null, true));
		...
		return '<div class="es-language-tabs">' . HTMLHelper::_('umartui.renderTab', 'tab-bottom') . '</div>';
	}
