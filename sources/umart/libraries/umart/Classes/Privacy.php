...
class Privacy extends PrivacyPlugin
{
	...
	public function onPrivacyCanRemoveData(PrivacyTableRequest $request, CMSUser $user = null)
	{
		...
		if ($user->authorise('core.admin', 'com_umart'))
		{
			$status->canRemove = false;
			$status->reason    = Text::_('COM_UMART_PRIVACY_DELETE_ADMIN_USER_DENIED');
		}
	...
	{
		$capabilities = [
			Text::_('COM_UMART_PRIVACY_BULLET_POINTS_DESC_1'),
			Text::_('COM_UMART_PRIVACY_BULLET_POINTS_DESC_2'),
			Text::_('COM_UMART_PRIVACY_BULLET_POINTS_DESC_3'),
		];

		PluginHelper::importPlugin('umart');
		PluginHelper::importPlugin('umart_payment');
		PluginHelper::importPlugin('umartshipping');
		plg_sytem_umart_main('app')->triggerEvent('onUmartPrivacyCollectAdminCapabilities', [&$capabilities]);

		return [
			Text::_('COM_UMART') => $capabilities,
		];
	}
	...
	public function onPrivacyExportRequest(PrivacyTableRequest $request, CMSUser $user = null)
	{
		...
		try
		{
			$utilityClass = plg_sytem_umart_main(Utility::class);

			// Export Customer Data
			$userClass           = $this->loadUserClass($user->id);
			$domain              = $this->createDomain('umart_customer', 'umart_customer_data');
			...
			$query = $this->db->getQuery(true)
				->select('a.id')
				->from($this->db->quoteName('#__umart_orders', 'a'))
			...
			if ($orderIds = $this->db->loadColumn())
			{
				...
				$domain     = $this->createDomain('umart_orders', 'umart_orders_data');
			...
			$query->clear()
				->select('a.*')
				->from($this->db->quoteName('#__umart_logs', 'a'))
				->innerJoin($this->db->quoteName('#__umart_users', 'a2') . ' ON a2.user_id = a.juser_id')
				->where('a2.id = ' . $user->id);
			$this->db->setQuery($query);

			if ($logs = $this->db->loadAssocList())
			{
				$domain = $this->createDomain('umart_logs', 'umart_logs_data');
	...
	public function onPrivacyRemoveData(PrivacyTableRequest $request, CMSUser $user = null)
	{
		...
		try
		{
			...
			$query->clear()
				->delete($this->db->quoteName('#__umart_customfield_values'))
				->where($this->db->quoteName('reflector') . ' = ' . $this->db->quote('com_umart.user'))
			...
			$subQuery = $this->db->getQuery(true)
				...
				->from($this->db->quoteName('#__umart_orders'))
				->where($this->db->quoteName('user_id') . ' = ' . $userId);
			$query->clear()
				->delete($this->db->quoteName('#__umart_customfield_values'))
				->where('(' . $this->db->quoteName('reflector') . ' = ' . $this->db->quote('com_umart.order.billing_address') . ' OR ' . $this->db->quoteName('reflector') . ' = ' . $this->db->quote('com_umart.order.shipping_address') . ')')
			...
			// Delete user logs
			$query->clear()
				->delete($this->db->quoteName('#__umart_logs'))
