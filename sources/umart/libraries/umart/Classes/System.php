...
class System
{
	...
	public function fixSchemas()
	{
		...
		$sqlInstallFiles = [
			UMART_COMPONENT_ADMINISTRATOR . '/sql/mysql/install.sql',
		];

		$app->triggerEvent('onUmartBeforeFixSchemas', [&$sqlInstallFiles]);
		$fileContents = '';
	...
	public function export()
	{
		...
		{
			plg_sytem_umart_main('app')->enqueueMessage('Class not exists: ZipArchive');
		...
		if (!is_dir(UMART_MEDIA . '/tmp'))
		{
			Folder::create(UMART_MEDIA . '/tmp', 0755);
		}

		$zipFile = UMART_MEDIA . '/tmp/Umart-export-data.' . date('Y-m-d-H-i-s') . '.zip';
		...
		if ($productsList = $this->getItemsFromDatabase('#__umart_products'))
		{
			...
			if ($media = $this->getItemsFromDatabase('#__umart_medias'))
			{
				foreach ($media as $file)
				{
					if (is_file(UMART_MEDIA . '/' . $file->file_path))
					{
						$files[] = $file;
						$zip->addFile(UMART_MEDIA . '/' . $file->file_path, 'media/' . $file->file_path);
					}
				}
			}
			...
			if ($data = $this->getItemsFromDatabase('#__umart_tag_items', 'item_id IN (' . implode(',', $ids) . ')'))
			{
				$zip->addFromString('tags.csv', $this->dataToCSV($data));
			}

			$tables = [
				'umart_customfields' => 'fields.csv',
				'umart_prices'       => 'prices.csv',
				'umart_price_days'   => 'priceDays.csv',
				'umart_taxes'        => 'taxes.csv',
			];
			...
			if ($data = $this->getItemsFromDatabase('#__umart_customfield_values', 'reflector = \'com_umart.product.customfield\''))
			{
				$zip->addFromString('fieldValues.csv', $this->dataToCSV($data));
			}

			if ($data = $this->getItemsFromDatabase('#__umart_tags', 'context = \'com_umart.product\''))
			{
				$zip->addFromString('tags.csv', $this->dataToCSV($data));
			}

			if ($data = $this->getItemsFromDatabase('#__umart_params', 'context LIKE \'%.product\''))
			{
	...
	public function getExpiredLayoutsFiles()
	{
		...
		{
			$path = JPATH_SITE . '/templates/' . $template . '/html/com_umart';
				...
				foreach (Folder::files($path, '\.php$', true, true) as $file)
				{
					...
					if (false !== $fp)
					{
						while (!feof($fp))
						{
							...
							if (!empty($matches[1])
								&& version_compare(UMART_VERSION, $matches[1], 'gt')
							)
							{
