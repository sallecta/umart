...
class Media
{
	...
	{
		$config = ComponentHelper::getParams('com_umart');
	...
	public function delete($filePath, $force = false)
	{
		...
		{
			$responseData['message'] = Text::_('COM_UMART_FILE_WAS_APPLIED_ON_PRODUCTS', true) . ': ';
			$productName             = [];
		...
		elseif (!is_file(UMART_MEDIA . '/' . $filePath) || !File::delete(UMART_MEDIA . '/' . $filePath))
		{
			$responseData['message'] = Text::sprintf('COM_UMART_REMOVE_FILE_FAIL', basename($filePath));
		}
		else
		{
			$responseData['success'] = true;
			$responseData['message'] = Text::sprintf('COM_UMART_REMOVE_FILE_SUCCESS', basename($filePath));
		}
	...
	public function getProductsByFile($filePath)
	{
		...
		$query = $db->getQuery(true)
			...
			->innerJoin($db->quoteName('#__umart_products', 'a2') . ' ON a2.id = a.product_id')
	...
	public function getFullImages($originImageBasePath)
	{
		...
		$image->image          = UMART_MEDIA_URL . '/' . $originImageBasePath;
		...
		if ($this->imageLazyResize)
		{
			...
			foreach ($sizes as $size)
			{
				...
				{
					$image->{$size} = $rootUrl . '/index.php?option=com_umart&task=ajax.loadImage&file=' . $file . '&size=' . $size;
				}
			}
		}
		else
		{
			...
			// @since 1.1.5. B/C
			if (is_file(UMART_MEDIA . $thumbPath . $fileName . '_tiny' . $ext))
			{
				$image->tiny = UMART_MEDIA_URL . $thumbPath . $fileName . '_tiny' . $ext;
			}
			else
			{
				$image->tiny = UMART_MEDIA_URL . $thumbPath . $fileName . '_' . $this->imageSizes['tinySize'] . $ext;
			}

			if (is_file(UMART_MEDIA . $thumbPath . $fileName . '_small' . $ext))
			{
				$image->small = UMART_MEDIA_URL . $thumbPath . $fileName . '_small' . $ext;
			}
			else
			{
				$image->small = UMART_MEDIA_URL . $thumbPath . $fileName . '_' . $this->imageSizes['smallSize'] . $ext;
			}

			if (is_file(UMART_MEDIA . $thumbPath . $fileName . '_medium' . $ext))
			{
				$image->medium = UMART_MEDIA_URL . $thumbPath . $fileName . '_medium' . $ext;
			}
			else
			{
				$image->medium = UMART_MEDIA_URL . $thumbPath . $fileName . '_' . $this->imageSizes['mediumSize'] . $ext;
			}

			if (is_file(UMART_MEDIA . $thumbPath . $fileName . '_large' . $ext))
			{
				$image->large = UMART_MEDIA_URL . $thumbPath . $fileName . '_large' . $ext;
			}
			else
			{
				$image->large = UMART_MEDIA_URL . $thumbPath . $fileName . '_' . $this->imageSizes['largeSize'] . $ext;
			}

			if (is_file(UMART_MEDIA . $thumbPath . $fileName . '_xlarge' . $ext))
			{
				$image->xlarge = UMART_MEDIA_URL . $thumbPath . $fileName . '_xlarge' . $ext;
			}
			else
			{
				$image->xlarge = UMART_MEDIA_URL . $thumbPath . $fileName . '_' . $this->imageSizes['xlargeSize'] . $ext;
			}
	...
	public function getResizeImageBasePath($originImageBasePath, $size, $autoResize = false)
	{
		...
		if ($autoResize && !is_file($cacheFilePath))
		{
			...
			if (!preg_match('/jpe?g|gif|png|svg|webp/i', $extension) || !is_file(UMART_MEDIA . '/' . $originImageBasePath))
			{
			...
			$jImage = new Image(UMART_MEDIA . '/' . $originImageBasePath);
			...
			$imgProperties = $jImage->getImageFileProperties(UMART_MEDIA . '/' . $originImageBasePath);
