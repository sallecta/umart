...
class Email
{
	...
	{
		$this->log = plg_sytem_umart_main(Log::class);
	...
	public function getEmails()
	{
		...
		if (null === $emails)
		{
			$emailsModel = plg_sytem_umart_main('model', 'Emails', UMART_COMPONENT_ADMINISTRATOR);
	...
	{
		$defaults = [
			'[ON_NEW_ORDER]'            => Text::_('COM_UMART_ON_NEW_ORDER'),
			'[ON_ORDER_CHANGE_STATE]'   => Text::_('COM_UMART_ON_ORDER_CHANGE_STATE'),
			'[ON_ORDER_CHANGE_PAYMENT]' => Text::_('COM_UMART_ON_PAYMENT_CHANGE_STATE'),
		];
	...
	public function sendEmail($email, $replaceData, Order $order = null)
	{
		...
		if (count($recipients))
		{
			...
			$this->app->triggerEvent('onUmartBeforeSendEmail', [$mailer, $email, $order]);
			$modifiedData = $mailer->Body;

			if ($mailer->Send())
			{
				$this->log->addEntry('com_umart.email', 'COM_UMART_EMAIL_SENT_SUCCESS', $sprintfData, $previousData, $modifiedData);
				$this->app->triggerEvent('onUmartAfterSendEmail', [$mailer, $email, $order]);
			}
			else
			{
				$this->log->addEntry('com_umart.email', 'COM_UMART_EMAIL_SENT_FAIL', $sprintfData, $previousData, $modifiedData);
			}
		}
	}
}
