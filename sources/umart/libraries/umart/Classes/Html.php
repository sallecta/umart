...
class Html
{
	...
	{
		$template           = plg_sytem_umart_main('app')->getTemplate();
		$this->includePaths = [
			JPATH_THEMES . '/' . $template . '/html/com_umart',
	...
	public function initChosen()
	{
		...
		if (!$chosen)
		{
			...
			plg_sytem_umart_main('doc')->addScriptDeclaration('
				jQuery(function ($) {
					_umart.initChosen = function(container) {
						var select,
							options = {
						...
						if (container){
						    ...
						    list = $(document).find(".umart_scope select");
						}
					...
					_umart.initChosen();
					$("body").on("subform-row-add", _umart.initChosen);
				});
			');
		}
	...
	protected function addMedia($file, $type, $version = null)
	{
		...
		{
			$mediaList[]  = $file;
			$includePaths = array_merge($this->includePaths, [UMART_MEDIA]);
			...
			if ($file = Path::find($includePaths, $type . '/' . $file))
			{
				...
				call_user_func_array([plg_sytem_umart_main('doc'), $callBack], [$file, ['version' => $version ? $version : null]]);
			}
		}
	...
	public function jui(array $ui = [])
	{
		...
		if (IS_JOOMLA_V4)
		{
			$doc->addScript(UMART_MEDIA_URL . '/js/ui/jquery.ui.core.min.js');
		}
		else
		{
		...
		if ($ui)
		{
			...
			{
				$doc->addScript(UMART_MEDIA_URL . '/js/ui/jquery.ui.' . $component . '.min.js');
			}

			if ($datePicker || $timePicker)
			{
				$appendJs = PHP_EOL . 'var dateTimeOpts = ' . json_encode([
						'closeText'          => Text::_('COM_UMART_UI_DATETIME_CLOSETEXT'),
						'prevText'           => Text::_('COM_UMART_UI_DATETIME_PREVTEXT'),
						'nextText'           => Text::_('COM_UMART_UI_DATETIME_NEXTTEXT'),
						'currentText'        => Text::_('COM_UMART_UI_DATETIME_CURRENTTEXT'),
						'monthNames'         => array_map('trim', explode(',', Text::_('COM_UMART_UI_DATETIME_MONTHSNAME'))),
						'monthNamesShort'    => array_map('trim', explode(',', Text::_('COM_UMART_UI_DATETIME_MONTHSNAMESHORT'))),
						'dayNames'           => array_map('trim', explode(',', Text::_('COM_UMART_UI_DATETIME_DAYNAMES'))),
						'dayNamesShort'      => array_map('trim', explode(',', Text::_('COM_UMART_UI_DATETIME_DAYNAMESSHORT'))),
						'dayNamesMin'        => array_map('trim', explode(',', Text::_('COM_UMART_UI_DATETIME_DAYNAMESMIN'))),
						'weekHeader'         => Text::_('COM_UMART_UI_DATETIME_WEEKHEADER'),
						'dateFormat'         => Text::_('COM_UMART_UI_DATETIME_DATEFORMAT'),
						'firstDay'           => (int) Text::_('COM_UMART_UI_DATETIME_FIRSTDAY'),
						'isRTL'              => $doc->direction === 'rtl',
						'showMonthAfterYear' => in_array(strtolower(Text::_('COM_UMART_UI_DATETIME_SHOWMONTHAFTERYEAR')), ['1', 'true', 'yes'], true),
						'yearSuffix'         => Text::_('COM_UMART_UI_DATETIME_YEARSUFFIX'),
						'amNames'            => array_map('trim', explode(',', Text::_('COM_UMART_UI_DATETIME_AMNAMES'))),
						'pmNames'            => array_map('trim', explode(',', Text::_('COM_UMART_UI_DATETIME_PMNAMES'))),
						'timeFormat'         => Text::_('COM_UMART_UI_DATETIME_TIMEFORMAT'),
						'timeSuffix'         => Text::_('COM_UMART_UI_DATETIME_TIMESUFFIX'),
						'timeOnlyTitle'      => Text::_('COM_UMART_UI_DATETIME_TIMEONLYTITLE'),
						'timeText'           => Text::_('COM_UMART_UI_DATETIME_TIMETEXT'),
						'hourText'           => Text::_('COM_UMART_UI_DATETIME_HOURTEXT'),
						'minuteText'         => Text::_('COM_UMART_UI_DATETIME_MINUTETEXT'),
						'secondText'         => Text::_('COM_UMART_UI_DATETIME_SECONDTEXT'),
						'millisecText'       => Text::_('COM_UMART_UI_DATETIME_MILLISECTEXT'),
						'microsecText'       => Text::_('COM_UMART_UI_DATETIME_MICROSECTEXT'),
						'timezoneText'       => Text::_('COM_UMART_UI_DATETIME_TIMEZONETEXT'),
					]) . ';';

				if ($datePicker)
				{
					$appendJs .= PHP_EOL . '_umart.$.datepicker.setDefaults(dateTimeOpts);';
				}

				if ($timePicker)
				{
					$appendJs .= PHP_EOL . '_umart.$.timepicker.setDefaults(dateTimeOpts);';
				}
	...
	public function flatPicker($selector = null, &$options = [])
	{
		...
		if (!$loaded)
		{
			...
			if (is_file(UMART_MEDIA . '/js/l10n/' . $lang . '.js'))
			{
				$this->addJs('l10n/' . $lang . '.js');
				plg_sytem_umart_main('doc')->addScriptDeclaration('flatpickr.localize(flatpickr.l10ns.' . $lang . ');');
			}
		}
