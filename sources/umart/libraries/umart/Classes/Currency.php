...
class Currency
{
	...
	protected $context = 'com_umart.currency';
	...
	public function __construct()
	{
		...
		if ($this->isMultiMode())
		{
			$id = (int) plg_sytem_umart_main('app')->input->post->getInt('umart_currency_id', 0);
	...
	public function getList()
	{
		...
		if (null === $items)
		{
			...
			$query = $db->getQuery(true)
				...
				->from($db->quoteName('#__umart_currencies', 'a'))
	...
	{
		return plg_sytem_umart_main('config', 'multi_currencies_mode', 0);
	}

	public function setActiveId($pk = 0)
	{
		plg_sytem_umart_main('app')->setUserState($this->context . '.active', $pk);
	}
	...
	public function load($pk)
	{
		...
		{
			throw new RuntimeException(Text::_('COM_UMART_ERROR_CURRENCY_NOT_FOUND'));
		}
	...
	public function getTable()
	{
		...
		return Table::getInstance('Currency', 'UmartTable');
	}
	...
	public function getActiveId()
	{
		...
		if ($this->isMultiMode())
		{
			$activeId = plg_sytem_umart_main('app')->getUserState($this->context . '.active', $defaultId);
		}
		else
		{
			$activeId = $defaultId;
