...
class Order
{
	...
	public function __construct()
	{
		...
		$this->table = Table::getInstance('Order', 'UmartTable');
	}
	...
	public function __get($name)
	{
		switch ($name)
		{
			...
			case 'currency':
				...
				{
					return plg_sytem_umart_main(Currency::class)->load($currencyId);
				}

				return plg_sytem_umart_main(Currency::class)->getDefault();
		}
	...
	public function getAddress($orderId = 0)
	{
		...
		/** @var $customField CustomField */
		$customField = plg_sytem_umart_main(CustomField::class, [
			'reflector' => 'com_umart.order.billing_address',
		]);
		$billing     = $customField->getValues($orderId, true);
		$customField = plg_sytem_umart_main(CustomField::class, [
			'reflector' => 'com_umart.order.shipping_address',
		]);
	...
	public function getFieldsPriceDetails($orderId = 0)
	{
		...
		$query = $db->getQuery(true)
			...
			->from($db->quoteName('#__umart_order_field_price_xref', 'a'))
	...
	public function save(&$data)
	{
		try
		{
			...
			if ($result = $this->table->store())
			{
				...
				$productQuery = $db->getQuery(true)
					->insert($db->quoteName('#__umart_order_products'))
					->columns(
						[
							'order_id',
							'product_id',
				...
				$optionQuery  = $db->getQuery(true)
					->insert($db->quoteName('#__umart_order_product_options'))
					->columns(
						[
							'order_product_id',
							'option_id',
				...
				if (!empty($data['discounts']))
				{
					...
					{
						$query = $db->getQuery(true)
							->delete($db->quoteName('#__umart_order_coupons'))
							->where($db->quoteName('order_id') . ' = ' . $orderId);
						$db->setQuery($query)
							->execute();

						$query->clear()
							->insert($db->quoteName('#__umart_order_coupons'))
		...
		$app->triggerEvent('onUmartAfterSave', ['com_umart.order', $this->table, $isNew, $data]);
		$app->triggerEvent('onUmartOrderAfterSave', [$this, $isNew]);
	...
	public function saveAddress($type, $address)
	{
		...
		$customField = plg_sytem_umart_main(CustomField::class,
			[
				'reflector'    => 'com_umart.order.' . $type . '_address',
				'reflector_id' => $orderId,
			]
		);
	...
	public function saveFieldPriceXref(array $fieldsValues)
	{
		...
		// Wipe old values
		$query = $db->getQuery(true)
			->delete($db->quoteName('#__umart_order_field_price_xref'))
		...
		if ($fieldsValues)
		{
			// Insert values
			$query->clear()
				->insert($db->quoteName('#__umart_order_field_price_xref'))
	...
	public function getProductDetails($orderId = 0)
	{
		...
		$query = $db->getQuery(true)
			...
			->leftJoin($db->quoteName('#__umart_products', 'a2') . ' ON a2.id = a.product_id')
			->from($db->quoteName('#__umart_order_products', 'a'))
		...
		if ($products = $db->loadObjectList())
		{
			...
			foreach ($products as $product)
			{
				...
				$query->clear()
					...
					->from($db->quoteName('#__umart_order_product_options', 'a'))
	...
	public function addProduct($productData, $options = [])
	{
		...
		{
			$result = $db->updateObject('#__umart_order_products', $dataObject, 'id');
		}
		else
		{
			$result = $db->insertObject('#__umart_order_products', $dataObject, 'id');
		}
		...
		if ($result && $options)
		{
			...
			foreach ($options as $option)
			{
				if (is_array($option) || is_object($option))
				{
					...
					$db->insertObject('#__umart_order_product_options', $option, 'id');
				}
	...
	public function updateProductStock()
	{
		...
		/** @var Log $logClass */
		Table::addIncludePath(UMART_COMPONENT_ADMINISTRATOR . '/tables');
		$productTable = Table::getInstance('Product', 'UmartTable');
		$logClass     = plg_sytem_umart_main(Log::class);
		...
		foreach ($this->products as $product)
		{
			if ($productTable->load($product->product_id))
			{
				...
				if ($currentStock !== -1)
				{
					...
					{
						$logClass->addEntry('com_umart.product', 'COM_UMART_PRODUCT_STOCK_UPDATED_FORMAT', [$productTable->name, $productTable->id, $stock], $currentStock, $stock);
					}
				}
			}
		}
	...
	{
		return [
			UMART_ORDER_CREATED   => Text::_('COM_UMART_ORDER_CREATED'),
			UMART_ORDER_CONFIRMED => Text::_('COM_UMART_ORDER_CONFIRM'),
			UMART_ORDER_PROCESSED => Text::_('COM_UMART_ORDER_PROCESSED'),
			UMART_ORDER_SHIPPED   => Text::_('COM_UMART_ORDER_SHIPPED'),
			UMART_ORDER_SUCCEED   => Text::_('COM_UMART_ORDER_SUCCEED'),
			UMART_ORDER_CANCELLED => Text::_('COM_UMART_ORDER_CANCELLED'),
			UMART_ORDER_ARCHIVED  => Text::_('COM_UMART_ORDER_ARCHIVED'),
			UMART_ORDER_TRASHED   => Text::_('COM_UMART_ORDER_TRASH'),
		];
	}
	...
	{
		return [
			UMART_PAYMENT_UNPAID => Text::_('COM_UMART_PAYMENT_UNPAID'),
			UMART_PAYMENT_PAID   => Text::_('COM_UMART_PAYMENT_PAID'),
			UMART_PAYMENT_REFUND => Text::_('COM_UMART_PAYMENT_REFUND'),
		];
	}
	...
	public function paid(array $extraData = [])
	{
		...
		{
			plg_sytem_umart_main('app')->triggerEvent('onUmartOrderPaid', [$this]);
			plg_sytem_umart_main(Log::class)->addEntry('com_umart.order', 'COM_UMART_LOG_ORDER_PAID', [$this->get('order_code')]);
			$this->updateProductStock();
		}
	...
	public function unpaid(array $extraData = [])
	{
		...
		{
			plg_sytem_umart_main('app')->triggerEvent('onUmartOrderUnpaid', [$this]);
			plg_sytem_umart_main(Log::class)->addEntry('com_umart.order', 'COM_UMART_LOG_ORDER_UNPAID', [$this->get('order_code')]);
		}
	...
	public function refund(array $extraData = [])
	{
		...
		{
			plg_sytem_umart_main('app')->triggerEvent('onUmartOrderRefund', [$this]);
			plg_sytem_umart_main(Log::class)->addEntry('com_umart.order', 'COM_UMART_LOG_ORDER_REFUND', [$this->get('order_code')]);
		}
	...
	public function updateCouponLimit($orderId, $orderStatus, $paymentStatus)
	{
		...
		$query   = $db->getQuery(true)
			...
			->from($db->quoteName('#__umart_order_coupons', 'a'))
			->innerJoin($db->quoteName('#__umart_discounts', 'a2') . ' ON a2.id = a.coupon_id')
		...
		if ($rows = $db->loadObjectList())
		{
			...
			foreach ($rows as $row)
			{
				...
				$query->clear()
					->update($db->quoteName('#__umart_discounts'))
				...
				$query->clear()
					->update($db->quoteName('#__umart_order_coupons'))
				...
				$logClass->addEntry('com_umart.discount', 'COM_UMART_COUPON_UPDATE_LIMIT_FORMAT', [$row->coupon_code, $oldLimit, $newLimit], $oldLimit, $newLimit);
			}
		}
	}
