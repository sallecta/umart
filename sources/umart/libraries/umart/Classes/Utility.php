...
class Utility
{
	...
	public function formatAddress($fieldData)
	{
		...
		$addressFormat = plg_sytem_umart_main('config', 'address_format', $defaultFormat);
	...
	public function findUpdate($reload = false)
	{
		...
		$result = $app->getUserState('com_umart.update', null);

		if ($reload || !is_array($result))
		{
			$result = [];
			$file   = UMART_COMPONENT_ADMINISTRATOR . '/umart.xml';
			...
			)
			{
				...
				)
				{
					...
					if (version_compare($version, UMART_VERSION, 'gt'))
					{
						$result = [
							'version'     => $version,
			...
			$app->setUserState('com_umart.update', $result);
		}
	...
	public function getShopInformation($vendorId = 0)
	{
		...
		plg_sytem_umart_main('app')->triggerEvent('onUmartPrepareShopInformation', [$vendorId, $fieldsMap, &$information]);
		...
		if ($information['shopCountry']
			...
			&& ($zoneTable = plg_sytem_umart_main(Zone::class)->load($information['shopCountry']))
		)
		{
	...
	public function getOrderingData()
	{
		...
		if (!is_array($sortArray))
		{
			$sortArray = [
				[
					'value' => 'ordering',
					'text'  => Text::_('COM_UMART_SORT_BY'),
				],
				[
					'value' => 'recent',
					'text'  => Text::_('COM_UMART_SORT_RECENT'),
				],
				[
					'value' => 'name_asc',
					'text'  => Text::_('COM_UMART_SORT_NAME_ASC'),
				],
				[
					'value' => 'name_desc',
					'text'  => Text::_('COM_UMART_SORT_NAME_DESC'),
				],
				[
					'value' => 'price_asc',
					'text'  => Text::_('COM_UMART_SORT_PRICE_ASC'),
				],
				[
					'value' => 'price_desc',
					'text'  => Text::_('COM_UMART_SORT_PRICE_DESC'),
				],
				[
					'value' => 'hits',
					'text'  => Text::_('COM_UMART_TOP_HITS'),
				],
			];

			plg_sytem_umart_main('app')->triggerEvent('onUmartPrepareOrderingData', [&$sortArray]);
		}
	...
	public function displayPicker($value, $options = [])
	{
		...
		$delimiter = 'range' === $options['mode'] ? UmartHelper::PICKER_RANGE_SEPARATOR : UmartHelper::PICKER_MULTIPLE_SEPARATOR;
		$dates     = [];
	...
	{
		$config = plg_sytem_umart_main('getConfig');
