...
class CustomField
{
	protected static $reflectors = [
		...
		'com_umart.order',
		'com_umart.checkout',
		'com_umart.product.customfield',
		'com_umart.product.option',
		'com_umart.order.billing_address',
		'com_umart.order.shipping_address',
	];
	protected $reflector;
	...
	public function load($key = 'id')
	{
		...
		if (!isset($fields[$ref]))
		{
			$showOn = (int) plg_sytem_umart_main('app')->getClientId();
	...
	public function check($full = false, $reflector = null)
	{
		...
		{
			throw new RuntimeException(Text::_('COM_UMART_WARNING_PROVIDE_VALID_REFLECTOR'), 404);
		}
	...
	{
		$key = 'COM_UMART_CUSTOMFIELD_' . strtoupper($label);
	...
	protected function parseFieldValues($field)
	{
		...
		switch ($type)
		{
			...
			case 'subzone':
				...
				if ($zoneTable = plg_sytem_umart_main(Zone::class)->load($field->value))
				{
	...
	public function validate($data = null, $groupName = 'customfields', $groupId = 0)
	{
		...
		{
			plg_sytem_umart_main('state')->set('customfield.errors', [$valid->getMessage()]);
		...
		if (false === $valid)
		{
			...
			plg_sytem_umart_main('state')->set('customfield.errors', $errors);
	...
	public function getForm($data = null, $groupName = 'customfields', $groupId = 0)
	{
		...
		$form        = new Form('com_umart.customfield', ['control' => 'jform']);
		...
		if (false !== $validateFields)
		{
			...
			foreach ($validateFields as $validateField)
			{
				...
				if (!@preg_match('/' . $regex . '/', $value))
				{
					if ($message)
					{
						plg_sytem_umart_main('state')->set('customfield.errors', [$message]);
					}
	...
	public function getValues($reflectorId = 0, $forceNew = false)
	{
		...
		if (!isset($values[$ref][$refId]) || $forceNew)
		{
			...
			$query = $db->getQuery(true)
				...
				->from($db->quoteName('#__umart_customfield_values', 'a'))
				->innerJoin($db->quoteName('#__umart_customfields', 'a2') . ' ON a2.id = a.customfield_id')
	...
	public function prepareField(DOMElement $parentField, $itemField, $name)
	{
		...
		switch (strtolower($itemField->type))
		{
			...
			case 'callname':
				$field->setAttribute('filter', 'UmartHelper::filterArrayToString');
			...
			case 'vat':
				$field->setAttribute('message', 'COM_UMART_CHECK_VAT_FAIL');
			...
			case 'flatpicker':
				...
				$field->setAttribute('filter', 'UmartHelper::filterDate' . ucfirst($mode));
	...
	public function insertValues(array $data = [])
	{
		...
		try
		{
			...
			if (count($values))
			{
				$this->removeValues();
				$query = $db->getQuery(true)
					->insert($db->quoteName('#__umart_customfield_values'))
		...
		{
			plg_sytem_umart_main('app')->enqueueMessage($e->getMessage(), 'warning');
	...
	public function removeValues($reflector = 0, $reflectorId = 0)
	{
		...
		$db    = plg_sytem_umart_main('db');
		$query = $db->getQuery(true)
			->delete($db->quoteName('#__umart_customfield_values'))
