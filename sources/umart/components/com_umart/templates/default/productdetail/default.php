...
{
	$mime    = $mediaClass->getMimeByFile(UMART_MEDIA . '/' . $file->file_path);
	...
	if ($isVideo)
	{
		...
		$mediaObject->source      = UMART_MEDIA_URL . '/' . $file->file_path;
...
$leftWidth    = $countMedia ? 'umartui_width-1-2@s umartui_width-1-3@m umartui_width-2-5@l' : 'umartui_width-1-1';
$rightWidth   = $countMedia ? 'umartui_width-1-2@s umartui_width-2-3@m umartui_width-3-5@l' : 'umartui_width-1-1';
 ...
 * @since 1.2.5
 */
$unpublished = plg_sytem_umart_main(User::class)->core('admin') && (int) $this->product->state !== 1;
...
<div id="product-detail" itemscope itemtype="http://schema.org/Product">
	...
	<?php HTMLHelper::_('umartui.addTab', Text::_('COM_UMART_DESCRIPTION'), 'es-icon-info'); ?>
	...
	<?php if ($config->get('product_detail_field', 'tab') == 'tab'
		...
		<?php foreach ($this->product->customfields as $customfield): ?>
			<?php if (count($customfield['fields'])): ?>
				<?php HTMLHelper::_('umartui.addTab', $customfield['title'], $customfield['params']->get('icon')); ?>
				<?php echo $renderer->render('product.field.' . $customfield['params']->get('fields_output_layout', 'table'), [
					'fields' => $customfield['fields'],
				]); ?>
				<?php HTMLHelper::_('umartui.endTab'); ?>
	...
	<?php echo HTMLHelper::_('umartui.renderTab', $config->get('product_detail_tab_layout', 'tab-default'), ['responsive' => true]); ?>
