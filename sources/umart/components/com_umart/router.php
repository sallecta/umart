...
class UmartRouter extends RouterBase
{
	public $app;
	public $menu;
	...
	protected function parseVars(&$vars, &$segments, $total, $item = null)
	{
		...
		)
		{
			...
			{
				$category = Categories::getInstance('umart.product')->get($cid);
		...
		if ($isMultiLanguage)
		{
			$query->join('LEFT', $db->quoteName('#__umart_translations', 'a2') . ' ON a2.translationId = CONCAT_WS(' . $db->quote('.') . ', ' . $db->quote($langCode) . ', ' . $db->quote('categories') . ', a.id, ' . $db->quote('path') . ')')
		...
		$query = $db->getQuery(true)
			...
			->from($db->quoteName('#__umart_products', 'a'))
			->join('INNER', $db->quoteName('#__categories', 'a2') . ' ON a2.id = a.category_id AND a2.extension = ' . $db->quote('com_umart.product'))
		...
		if ($isMultiLanguage)
		{
			$query->join('LEFT', $db->quoteName('#__umart_translations', 'a3') . ' ON a3.translationId = CONCAT_WS(' . $db->quote('.') . ', ' . $db->quote($langCode) . ', ' . $db->quote('umart_products') . ', a.id, ' . $db->quote('alias') . ')')
		...
		{
			$category = Categories::getInstance('umart.product')->get($obj->category_id);
...
function umartBuildRoute(&$query)
{
	$router = new UmartRouter;

	return $router->build($query);
}

function umartParseRoute($segments)
{
	$router = new UmartRouter;
