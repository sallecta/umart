...
class UmartControllerCheckout extends BaseController
{
	/**
	 * @var Cart
	 * @since 1.0.0
	...
	public function saveAddress()
	{
		try
		{
			...
			$response = [
				'pushState' => [
					'url'   => Route::_(UmartHelperRoute::getCartRoute('confirm'), false),
					'title' => Text::_('COM_UMART_PAGE_CHECKOUT_CONFIRM_TITLE'),
				],
	...
	protected function getCustomerForm($bindData = null)
	{
		...
		if (!CMSFactory::getUser()->id)
		{
			...
			if ($config->get('enable_registration', 1))
			{
				...
				{
					return new RuntimeException(Text::_('COM_UMART_MUST_LOGIN_BEFORE_CHECKOUT_MSG'));
				}
			}
			...
			if ($form->validate($bindData, 'registration'))
			{
				if ($register)
				{
					...
					{
						return new RuntimeException(Text::sprintf('COM_UMART_USER_EMAIL_EXISTS_MSG', $email));
					}
				}
			}
			else
	...
	public function calculateCheckoutFieldsPrice($responseData = true, $groups = ['checkoutFields'])
	{
		try
		{
			...
			if (!empty($data['checkoutFields']))
			{
				 ...
				 * @var Tax         $taxClass
				 */
				$taxClass   = plg_sytem_umart_main(Tax::class);
				$fieldClass = plg_sytem_umart_main(CustomField::class, ['reflector' => 'com_umart.checkout']);
	...
	protected function validate($groups = null, $throw = false)
	{
		...
		if ($task === 'saveAddress')
		{
			if (empty($postData['address_different']) || plg_sytem_umart_main('config', 'disable_shipping_address', 0))
			{
		...
		try
		{
			plg_sytem_umart_main('app')->triggerEvent('onUmartAfterValidateCheckoutData', [&$data]);
		}
		catch (RuntimeException $e)
		{
			if ($throw)
	...
	{
		$controller = BaseController::getInstance('Umart', ['base_path' => UMART_COMPONENT_SITE]);
		$view       = $controller->getView('Cart', 'html', 'UmartView', ['base_path' => UMART_COMPONENT_SITE . '/views']);
	...
	public function editAddress()
	{
		try
		{
			...
			$response = [
				'pushState' => [
					'url'   => Route::_(UmartHelperRoute::getCartRoute('checkout'), false),
					'title' => Text::_('COM_UMART_PAGE_CHECKOUT_ADDRESS_TITLE'),
				],
	...
	public function guest()
	{
		...
		if (plg_sytem_umart_main('config', 'guest_checkout', 1))
		{
			...
			{
				$this->app->enqueueMessage(Text::_('COM_UMART_ERROR_INVALID_EMAIL'), 'error');
			}
			else
			{
			...
			{
				$return = Route::_(UmartHelperRoute::getCartRoute('checkout'));
			}
		}
	...
	public function finish()
	{
		...
		$redirect       = Route::_(UmartHelperRoute::getCartRoute('confirm'), false);
		$extractData    = null;
		$paymentHandler = null;
		...
		try
		{
				...
				if ($fields = $form->getGroup($groupField))
				{
					foreach ($fields as $field)
					{
						...
						if (!empty($validateRegex) && !@preg_match('/' . $validateRegex . '/', $fieldValue))
						{
							...
							throw new RuntimeException($regexMessage ?: Text::_('COM_UMART_INPUT_INVALID_REGEX'));
						}
					}
				}
			}

			if (plg_sytem_umart_main('config', 'terms_and_conditions', 1) && empty($data['confirm']['terms_and_conditions']))
			{
				throw new RuntimeException(Text::_('COM_UMART_TERMS_AND_CONDITIONS_CONFIRM_WARNING'));
			}

			if (empty($data['billing_address']) || empty($data['shipping_address']))
			{
				throw new RuntimeException(Text::_('COM_UMART_ERROR_INVALID_ADDRESS'));
			}

			plg_sytem_umart_main('state')->set('checkoutStep', 'finishing');
			...
			{
				throw new RuntimeException(Text::_('COM_UMART_YOUR_CART_EMPTY'));
			}

			$paymentMethods  = $this->cart->getPaymentMethods();
			$shippingMethods = $this->cart->getShippingMethods();
			$app->triggerEvent('onUmartCheckoutPrepareCartData', [&$extractData]);

			if (!($payment = $this->cart->getPaymentMethods(true)) && $paymentMethods)
			{
				throw new RuntimeException(Text::_('COM_UMART_ERROR_NO_PAYMENT'));
			}
			...
			if (is_object($payment))
			{
				...
				{
					throw new RuntimeException(Text::_('COM_UMART_ERROR_INVALID_PAYMENT_HANDLER'));
				}
				...
				{
					throw new RuntimeException(Text::sprintf('COM_UMART_ERROR_INVALID_PAYMENT_CURRENCY_FORMAT', $payment->name, $currencyCode));
				}
				...
				{
					throw new RuntimeException(Text::_('COM_UMART_ERROR_PAYMENT_VALIDATE_FAIL'));
				}
			}

			if (!($shipping = $this->cart->getShippingMethods(true)) && $shippingMethods)
			{
				throw new RuntimeException(Text::_('COM_UMART_ERROR_NO_SHIPPING'));
			}
			...
			else
			{
				...
				{
					throw new RuntimeException(Text::_('COM_UMART_ERROR_USER_EMAIL'));
				}
				...
				if ($customerForm->getValue('register', 'registration'))
				{
					...
					$this->getModel('Customer', 'UmartModel')->register($registerUserData, true);
			...
			$app->triggerEvent('onUmartCheckoutPrepareOrderData', [$order, &$orderData]);
			...
			if (!$order->save($orderData))
			{
				...
				throw new RuntimeException(Text::_('COM_UMART_ORDER_SAVE_FAIL_MESSAGE'));
			}
			// Fire the event before payment
			$message = trim(implode(PHP_EOL, $app->triggerEvent('onUmartOrderCreated', [$order])));
			...
			// Email sending
			$emailClass = plg_sytem_umart_main(Email::class);
			$emailClass->sendOn('[ON_NEW_ORDER]', $order->getLayoutData(), $order);
			$data['finalData'] = plg_sytem_umart_main('renderer')->render('checkout.message.success',
				[
					'order'   => $order,
					'message' => $message,
				]
			...
			if (empty($data['finalData']))
			{
				...
				$app->enqueueMessage(Text::sprintf('COM_UMART_CHECKOUT_SUCCESS_MESSAGE', $orderData['order_code']), 'success');
			}

			// Fire the event after render success message and redirect to the finished page.
			$app->triggerEvent('onUmartOrderFinished', [$order, &$data['finalData']]);
	...
	protected function addMethod($type)
	{
		...
		try
		{
			...
			if (!$found)
			{
				throw new RuntimeException(Text::_('COM_UMART_ERROR_INVALID_' . strtoupper($type) . '_ID'));
			}
